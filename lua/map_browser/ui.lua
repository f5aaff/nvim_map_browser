...
local function get_existing_map()
    ...
    for _, mode in ipairs(modes) do
        for _, map in ipairs(vim.api.nvim_get_keymap(mode)) do
            ...
            local display = string.format("%s | %s → %s | ",mode, map.lhs, map.rhs or "", desc)
...
if row < 0 then row = 0 end  -- ensure it doesn't go off-screen
    ...
    vim.api.nvim_buf_set_keymap(input_buf, "i", "<Down>", "<Cmd>lua require('map_browser.ui').move_selection(1)<CR>",
        { noremap = true, silent = true })
    vim.api.nvim_buf_set_keymap(input_buf, "i", "<Up>", "<Cmd>lua require('map_browser.ui').move_selection(-1)<CR>",
        { noremap = true, silent = true })
    vim.api.nvim_buf_set_keymap(input_buf, "i", "<CR>",
        "<Cmd>lua require('map_browser.ui').select_current_item()<CR>",
        { noremap = true, silent = true })
    vim.api.nvim_buf_set_keymap(input_buf, "i", "<Esc>", "<Cmd>lua require('map_browser.ui').force_close()<CR>",
        { noremap = true, silent = true })
end
...
function M.update_results()
    ...
    for _, filepath in ipairs(get_existing_map()) do
...
function M.open_map_browser()
    M.open_search_window(get_existing_map())
end
